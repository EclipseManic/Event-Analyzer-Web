...
def create_app() -> Flask:
    ...
    def delete_investigation(inv_id: str):
        ...
        thread = threading.Thread(target=_purge, name=f"delete-{inv_id[:8]}")
        thread.start()
        return jsonify({"status": "deleting"}), 202
    ...
    def get_events(inv_id: str):
        ...
        count = db.get_event_count(inv_id, filters=filters)
        return jsonify({"items": rows, "count": count})
    ...
    def upload_evtx():
        ...
        def _runner(sem: threading.Semaphore) -> None:
            try:
                sem.acquire()
                ingest_files(
        ...
        thread = threading.Thread(target=_runner, args=(_INGEST_SEM,), name=f"ingest-{inv_id[:8]}")
        thread.start()
    ...
    def get_iocs(inv_id: str):
        page = _safe_int(request.args.get("page"), 1)
        per_page = _safe_int(request.args.get("per_page"), 50)
        per_page = max(1, min(per_page, 500))
        offset = (page - 1) * per_page
        ioc_type = request.args.get("type", "")
        items = db.get_iocs(inv_id, limit=per_page, offset=offset, ioc_type=ioc_type)
        total = db.count_iocs(inv_id, ioc_type=ioc_type)
        return jsonify({"items": items, "total": total, "page": page, "per_page": per_page})

    @app.get("/api/investigations/<inv_id>/sigma")
    def get_sigma(inv_id: str):
        page = _safe_int(request.args.get("page"), 1)
        per_page = _safe_int(request.args.get("per_page"), 20)
        per_page = max(1, min(per_page, 100))
        offset = (page - 1) * per_page
        level = request.args.get("level", "")
        items = db.get_sigma_grouped(inv_id, limit=per_page, offset=offset, level=level)
        total = db.count_sigma_groups(inv_id, level=level)
        return jsonify({"items": items, "total": total, "page": page, "per_page": per_page})

    @app.get("/api/investigations/<inv_id>/sigma/events")
    def sigma_rule_events(inv_id: str):
        rule_id = request.args.get("rule_id", "")
        if not rule_id:
            return jsonify({"error": "rule_id required"}), 400
        info = db.get_sigma_rule_info(inv_id, rule_id)
        events = db.get_sigma_rule_events(inv_id, rule_id)
        return jsonify({"rule": info, "events": events})

    @app.get("/api/investigations/<inv_id>/iocs/summary")
    def get_ioc_summary(inv_id: str):
        return jsonify(db.get_ioc_summary(inv_id))

    @app.get("/api/investigations/<inv_id>/iocs/export")
    def export_iocs(inv_id: str):
        fmt = (request.args.get("format") or "csv").strip().lower()
        ioc_type = request.args.get("type", "")
        items = db.get_all_iocs(inv_id, ioc_type=ioc_type)
        if fmt == "json":
            return Response(
                json.dumps(items, indent=2),
                mimetype="application/json",
                headers={"Content-Disposition": f"attachment; filename={inv_id}_iocs.json"},
            )
        ...
        def generate():
            ...
            writer.writerow(["ioc_type", "value", "source", "event_count", "context"])
            yield output.getvalue()
            output.seek(0)
            output.truncate(0)
            for row in items:
                writer.writerow([row.get("ioc_type"), row.get("value"), row.get("source"), row.get("event_count"), row.get("context")])
                yield output.getvalue()
                output.seek(0)
                output.truncate(0)
        ...
        return Response(
            generate(),
            mimetype="text/csv",
            headers={"Content-Disposition": f"attachment; filename={inv_id}_iocs.csv"},
        )

    @app.get("/api/investigations/<inv_id>/sigma/summary")
    def get_sigma_summary(inv_id: str):
        return jsonify(db.get_sigma_summary(inv_id))

    @app.get("/api/investigations/<inv_id>/sigma/export")
    def export_sigma(inv_id: str):
        fmt = (request.args.get("format") or "csv").strip().lower()
        level = request.args.get("level", "")
        items = db.get_sigma_grouped(inv_id, limit=9999, offset=0, level=level)
        if fmt == "json":
            return Response(
                json.dumps(items, indent=2),
                mimetype="application/json",
                headers={"Content-Disposition": f"attachment; filename={inv_id}_sigma.json"},
            )
        import csv as csv_mod
        ...
        def generate():
            output = io.StringIO()
            writer = csv_mod.writer(output)
            writer.writerow(["rule_id", "rule_title", "level", "match_count", "description", "mitre_techniques"])
            yield output.getvalue()
            output.seek(0)
            output.truncate(0)
            for row in items:
                mt = "; ".join(f"{t.get('id','')} {t.get('name','')}" for t in (row.get("mitre_techniques") or []))
                writer.writerow([row.get("rule_id"), row.get("rule_title"), row.get("level"), row.get("match_count"), row.get("description"), mt])
                yield output.getvalue()
                output.seek(0)
                output.truncate(0)
        ...
        return Response(
            generate(),
            mimetype="text/csv",
            headers={"Content-Disposition": f"attachment; filename={inv_id}_sigma.csv"},
        )

    @app.post("/api/investigations/<inv_id>/bookmarks")
    def add_bookmark(inv_id: str):
        data = request.get_json(force=True)
        event_id = data.get("event_id", "")
        if event_id:
            db.add_bookmark(inv_id, event_id)
        return jsonify({"status": "ok"})

    @app.delete("/api/investigations/<inv_id>/bookmarks/<event_id>")
    def remove_bookmark(inv_id: str, event_id: str):
        db.remove_bookmark(inv_id, event_id)
        return jsonify({"status": "ok"})

    @app.get("/api/investigations/<inv_id>/bookmarks")
    def get_bookmarks(inv_id: str):
        return jsonify({"ids": db.get_bookmarked_ids(inv_id)})

    @app.get("/api/investigations/<inv_id>/notes")
    def get_notes(inv_id: str):
        content = db.get_note(inv_id)
        return jsonify({"content": content or ""})

    @app.put("/api/investigations/<inv_id>/notes")
    def save_notes(inv_id: str):
        data = request.get_json(force=True)
        content = (data.get("content") or "").strip()
        db.save_note(inv_id, content)
        return jsonify({"status": "ok"})

    @app.get("/api/investigations/<inv_id>/report")
    def get_report(inv_id: str):
        html = generate_report(inv_id)
        return Response(html, mimetype="text/html", headers={"Content-Disposition": f"attachment; filename={inv_id}_report.html"})

    @app.cli.command("update-sigma")
    def update_sigma_cli():
        """Download and update Sigma community rules."""
        from app.sigma_updater import run
        run()

    @app.cli.command("backfill")
    def backfill_cli():
        """Re-extract IOCs and Sigma matches for all investigations."""
        from app.backfill import run
        run()
